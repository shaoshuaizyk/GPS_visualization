...
# GPS数据记录和可视化类
class GPSVisualizer:
    def __init__(self, start_lat_on_map, start_lon_on_map, device, baud=115200, max_points=1200):
    ...
    def start(self):
        ...
        # 绘制地图起点和终点
        ax.plot([self.start_easting_on_map-ref_x, end_easting_on_map-ref_x],[self.start_northing_on_map-ref_y, end_northing_on_map-ref_y], 'ro--', markersize=8, label='地图参考起点/终点')
        ...
        ax.plot([start_utm[0]-ref_x], [start_utm[1]-ref_y], 'go', markersize=8, label='起点')
        ax.plot([end_utm[0]-ref_x], [end_utm[1]-ref_y], 'ko', markersize=8, label='终点')
...
# 主程序
def main(lat_on_map, lon_on_map):
    print_header()
    ...
    # 创建并启动可视化器
    visualizer = GPSVisualizer(lat_on_map, lon_on_map, device, baud)
    visualizer.start()
